...
it = 0
# cost_fun_k = 10*cost_functional_proj(uk, zeros_nt, ck, zeros_nt, 0, uhat_all, 
#                                      num_steps, dt, M, c_lower, c_upper, beta)
cost_fun_k = 10* cost_functional(uk, uhat_all, ck, num_steps, dt, M, beta)
...
while(RE_costfun >= tol) and it < 1000:
    ...
    cost_fun_k = cost_functional(uk, uhat_all, ck, num_steps, dt, M, beta)
    ...
    while cost_dif > - gam / s * stat_measure and k < max_iter:
        ...
        #######################################################################
        cost_fun_armijo = cost_functional(uk, uhat_all, c_inc, num_steps, dt, M, beta)
    ...
    cost_fidelity_vals.append(L2_norm_sq_Q(uk - uhat_all, num_steps, dt, M))
    ...
    for i in range(num_steps):
        ...
        if show_plots is True and i%5 == 0:
            ...
            im1 = plt.imshow(uhat_all_re_t)
            fig2.colorbar(im1)
            plt.title(f'{it=}, Desired state for $u$ at t = {round(tU,5)}')
            fig2.tight_layout(pad = 3.0)
...
for i in range(num_steps):
    ...
    #     im1 = plt.imshow(uhat_all_re_t)
    #     fig2.colorbar(im1)
    #     plt.title(f'Desired state for $u$ at t = {round(tU,5)}')
    #     fig2.tight_layout(pad = 3.0)
    #     ax2 = plt.subplot(1,4,2)
    #     im1 = plt.imshow(u_re)
    #     fig2.colorbar(im1)
